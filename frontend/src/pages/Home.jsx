...
import { ArrowRight, Play, Sparkles, CheckCircle2, XCircle } from "lucide-react";
...
export default function Home() {
  ...
  useLayoutEffect(() => {
    ...
    if (videoElRef.current) videoElRef.current.playbackRate = 0.4;
    ...
    const ctx = gsap.context(() => {
      ...
      rewriteTl
        .from(".rewrite-card-bad", { x: -50, opacity: 0, duration: 0.6, ease: "back.out" })
  ...
  return (
    <div ref={containerRef} className="relative min-h-screen bg-white p-4 md:p-6 lg:p-8 font-sans overflow-x-hidden">
      
      {/* --- PRELOADER --- */}
      <div ref={loaderRef} className="fixed inset-0 z-50 flex items-end justify-between bg-[#0a0a0a] px-6 py-8 text-white md:px-12">
        <div className="text-sm uppercase tracking-widest text-white/50">Creatdiv AI <br /> Est. 2024</div>
        <div className="text-9xl font-bold leading-none tracking-tighter opacity-20 text-white">{progress}%</div>
      </div>
      ...
      <section className="hero-frame relative h-[85vh] w-full overflow-hidden rounded-[32px] md:rounded-[48px] shadow-sm mb-24 bg-black">
        <div ref={videoWrapperRef} className="absolute inset-0 -top-[10%] h-[120%] w-full">
          <video ref={videoElRef} src={HeroVideo} autoPlay loop muted playsInline className="h-full w-full object-cover opacity-80" />
        </div>
        ...
        <div className="relative z-10 flex h-full flex-col items-center justify-center text-center px-4">
          ...
          <div className="overflow-hidden"><h1 className="title-word text-[12vw] md:text-[8rem] font-semibold leading-[0.9] tracking-tighter text-white">Creatdiv</h1></div>
          <div className="overflow-hidden"><h1 className="title-word text-[12vw] md:text-[8rem] font-semibold leading-[0.9] tracking-tighter text-white">Visuals</h1></div>
        ...
        <nav className="hero-fade absolute top-0 left-0 w-full p-6 md:p-8 flex justify-between text-white mix-blend-difference z-20">
          <div className="text-2xl font-bold tracking-tighter">Creatdiv.</div>
          <button className="hidden md:block rounded-full border border-white/20 px-6 py-2 text-sm hover:bg-white hover:text-black transition">Sign In</button>
        </nav>
      </section>
      ...
      <section className="demo-section relative py-20">
        <div className="text-center mb-16 px-4">
            ...
            <h2 className="text-4xl md:text-6xl font-bold text-gray-900 tracking-tight leading-tight">Turn words into <br/> worlds instantly.</h2>
        </div>
        ...
        <div className="demo-video-container mx-auto w-full max-w-[90vw] h-[60vh] md:h-[80vh] overflow-hidden shadow-2xl relative rounded-[32px] bg-black">
            <video src={DemoVideo} autoPlay loop muted playsInline className="w-full h-full object-cover opacity-90" />
         ...
         <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 md:grid-cols-2 gap-20 items-center">
            ...
            <div>
                ...
                <button className="group flex items-center gap-2 text-blue-600 font-bold hover:text-blue-700 transition-colors">
                    Try Rewriting <ArrowRight size={18} className="group-hover:translate-x-1 transition-transform" />
                </button>
            </div>
            ...
            <div className="relative perspective-1000">
                ...
                <div className="relative flex flex-col gap-6">
                    ...
                    <div className="rewrite-card-good relative bg-white p-6 rounded-2xl shadow-xl border border-blue-100 flex gap-4 items-start scale-105">
                        ...
                        <div>
                            <p className="text-gray-900 font-medium text-lg leading-snug">"Could you please review this draft? I believe it requires improvement."</p>
      ...
      <section className="py-24 px-4 max-w-[90vw] mx-auto">
         <div className="flex justify-between items-end mb-12">
             <h2 className="text-4xl font-bold text-gray-900 tracking-tight">Made with Creatdiv</h2>
             <span className="hidden md:block text-sm font-medium text-gray-400">Scroll to explore</span>
         </div>
         
         <div className="gallery-grid grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
             <GalleryItem src="https://images.unsplash.com/photo-1620641788421-7a1c342ea42e?w=800&q=80" title="Neon Cyberpunk" />
             <GalleryItem src="https://images.unsplash.com/photo-1635322966219-b75ed3a90164?w=800&q=80" title="Abstract Fluid" />
             <GalleryItem src="https://images.unsplash.com/photo-1699478726597-94b150c265ec?w=800&q=80" title="Space Nebula" />
             <GalleryItem src="https://images.unsplash.com/photo-1618005182384-a83a8bd57fbe?w=800&q=80" title="Glass Prism" />
         </div>
      </section>

      <div className="h-20 flex items-center justify-center text-gray-300 text-sm">© 2024 Creatdiv Inc.</div>
    </div>
  );
}
...
const GalleryItem = ({ src, title }) => (
    <div className="gallery-item group relative overflow-hidden rounded-2xl aspect-[3/4] cursor-pointer">
        ...
        <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-end p-6">
            <span className="text-white font-medium transform translate-y-4 group-hover:translate-y-0 transition-transform duration-300">{title}</span>
        </div>
    </div>
);